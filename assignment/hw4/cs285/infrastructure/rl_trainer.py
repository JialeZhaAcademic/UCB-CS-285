...
class RL_Trainer(object):
    ...
    def train_sac_agent(self):
        ...
        for _ in range(self.sac_params['num_agent_train_steps_per_iter']):
            ob_no, ac_na, re_n, next_ob_no, terminal_n = self.agent.sample_sac(
                self.sac_params['train_batch_size'])
            
            log = self.agent.train_sac(ob_no, ac_na, re_n, next_ob_no, terminal_n)
            sac_logs.append(log)
        return sac_logs
